...
Gängiges Verhalten, ist eine Software in mehrer Zustände zu unterteilen, und diese auf einem Stapel abzulegen. Wird nun z.B. das Pause-Menü geöffnet, wird der Zustand dieses Menüs auf den Stapel gepackt. Wenn das Menü wieder geschlossen wird, wird der darunterliegende Zustand des Spiels einfach weiter geführt. Dies ist hier auch implementiert, jedoch gibt es momentan nur einen Zustand.

Der einzige Zustand, den unser Spiel momentant kennt ist der PlayState. Er enthält die Welt, den Spieler, grundlegende Konfigurationen des Spiels und eine Referenz zur Eingabe von Tasten.



\subsubsection*{Player}

Die \texttt{Player} Klasse stellt eine Schnittstelle für den Benutzer dar, mit dem Spiel zu interagieren. Sie handhabt Tastatureingaben zum bewegen in der Welt als auch Mausbewegungen, um sich umschauen zu können oder auch mit der Welt zu interagieren. Diese Interaktionen beinhalten zum Beispiel das Abbauen und Platzieren von Blöcken in der Welt. An den Spieler ist auch die Kamera befestigt, durch welche der Benutzer die Welt sehen kann.
...
Das Rendering des Spiels ist eine komplexe Angelegenheit, und wir von uns im Laufe unserer Änderungen nicht verändert und als gegeben angesehen. Aufgruind der Komplexität ist es auch nicht vollständig modelliert, soll dennoch hier etwas genauer erklärt werden.

Die Application hat eine Referenz zum sog. RenderMaster, die von der Application zu den jeweiligen Klassen weitergereicht wird, die diesen Aufrfen wollen. 
...
Unser Projekt benutzt verschiedene Bobliotheken, die oft in solchen \texttt{C++} Projekten vorkommen. Für komplexere Rechnungen zieht der Programmierer hier \texttt{GLM} heran. Davon benutzt das Programm unterschiedlichen Vektoren, Materialien und Matrizen oder wird für z.B. Strahlenrechnung benutzt.
...
\texttt{Glad} ist die Bibliothek, die das Rendern mit OpenGL übernimmt und wird in den Render Klassen aufgerufen, um mithilfer des Meshes und Modelle und den Shadern das Spiel darzustellen.

\subsection{Zustandsmodell} \label{subsec:statemodel}

In unserem Projekt bietet sich nicht an, Zustandsdiagramme zu ertellen. Viele Prozesse laufen sehr statisch ab und würden in zu simplen Zustandsdiagrammen resultieren.
...
Ein weiteres Problem, dass bei verbleibender Zeit gelöst werden könnte, ist das Speichersystem. Momentan werden jegliche Änderungen and den Blockkonfigurationen der Chunks verworfen, wenn der Chunk entladen wird. Wird er neu geladen wird er neu generiert und die Änderungen gehen verloren. 

Folgende Änderungen können hier getätigt werden:

\begin{description}
  \item[Speichern bei Laufzeit:] Die erste Funktion wäre herauszufinden, wie die Änderungen platzeffizient gespeichert werden könnten, und wie man sie dann während des Spiels speichern und laden kann
  \item[Speichern und Laden nach Laufzeit:] Weiterführend kann man diese Änderungen auch als Datei speichern, sodass nach Beendung des Spiels die gespeicherte welt wieder geöffnet werden kann. Dies bracuht dann einige zusätzliche Features der Bedienung.
