...
Die \texttt{Player} Klasse stellt eine Schnittstelle für den Benutzer dar, mit dem Spiel zu interagieren und ist von der Klasse \texttt{Entity} abgeleitet. Sie ist eng verknüpft mit der Klasse \texttt{PlayState} und somit auch \texttt{Keyboard}. Dadurch werden die Tastatureingaben zum bewegen in der Welt verarbeitet. So auch die Mausbewegungen, um sich umschauen zu können, oder Interaktionen mit der Welt vornehmen zu können. Diese Interaktionen beinhalten beispielsweise das Abbauen und Platzieren von Blöcken in der Welt. Baut der Spieler einen Block ab, so wird er als Item, mit spezifischen Material, in der Klasse \texttt{ItemStack} gespeichert. Durch die \texttt{Kamera}, welcher der Spieler angehört, kann der Benutzer die Welt sehen.
